...
| img.height;
  ...
  const onResize = debounce(applyZoomAndCenter);

  if (img.complete) {
    applyZoomAndCenter();
  } else {
    img.addEventListener('load', applyZoomAndCenter);
  }
  ...
  window.addEventListener('orientationchange', () => {
    setTimeout(adjustViewportHeight, 120);
  });
  if (img) img.addEventListener('load', adjustViewportHeight);
