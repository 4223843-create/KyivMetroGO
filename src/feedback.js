...
MetroApp.triggerFeedbackSubmit = async function(background = false) {
  ...
  try {
    ...
    // ── Лог змін для режиму розробника ──
    if (isDevMode()) {
...
document.getElementById('fbStation').value = '';
    ...
    // 3. АВТОМАТИЧНО ТИСНЕМО на фільтр "Всі", щоб показати список станцій одразу
