...
s?.positions?.forEach((p, i) => {
  const safeDir = p.dir || '';
  let g = groupsMap.get(safeDir);
  if (!g) { g = { dir: safeDir, items: [] }; groupsMap.set(safeDir, g); }
  g.items.push({ p, i });
});
...
Object.keys(fbState.current).forEach(idx => {
  ...
  if (st.isNew) {
    const safeDir = st.dir || '';
    let g = groupsMap.get(safeDir);
    if (!g) { g = { dir: safeDir, items: [] }; groupsMap.set(safeDir, g); }
    g.items.push({ p: { dir: safeDir, exit: fbState.labels[idx] || '', wagon: st.wMain, doors: st.dMain }, i: parseInt(idx) });
  }
});
...
posEl.innerHTML = groups.map(g => {
  ...
  const itemsHtml = g.items.map((item, index) => {
    ...
    const rawExit = String(fbState.labels[item.i] || '');
    const safePencil = MetroApp.Icons?.pencil || PENCIL_SVG_LABEL;
    const safeInfo = MetroApp.Icons?.info || 'i';
    const safeUndo = MetroApp.Icons?.undo || '↺';
    ...
    const exitLabelHtml = '<div class="fb-exit-label-row">'
      ...
      + (rawExit
        ? '<button type="button" class="fb-exit-label-edit-btn" data-item-idx="' + item.i + '" aria-label="Редагувати">' + safePencil + '</button>'
    ...
    <div class="fb-item-inner ${st.hasExtra ? 'fb-pos-multi has-extra-doors' : ''} ${st.hasThird ? 'has-three-doors' : ''} ${isClosed ? 'fb-pos-closed' : ''}" data-idx="${item.i}" id="fbItemInner${item.i}">
      ${isClosed
        ? `<div class="fb-closed-note-wrap"><span class="fb-closed-note">Вихід позначено як недоступний</span><button type="button" class="fb-restore-exit" data-idx="${item.i}" aria-label="Відновити вихід">${safeUndo}</button></div>`
        : `<div class="fb-pos-wrap"><div class="fb-side-actions-left"><button type="button" class="fb-add-doors-info" style="color:${fbLineColor}" data-idx="${item.i}">${safeInfo}</button></div>
...
posEl.innerHTML = `<div style="color:var(--line-red); padding:16px; text-align:center; font-size:14px; background:var(--bg-card); border-radius:12px;">Помилка рендеру:<br>${err.message}</div>`;
console.error('[FeedbackSheet] Render error:', err);
}
}

// Зберігаємо на MetroApp для зворотньої сумісності (devmode.js)
MetroApp._renderFeedbackPositions = renderFeedbackPositions;

async function triggerFeedbackSubmit(background = false) {
...
if (isDevMode()) {
  posChanges.forEach(c => {
    ...
    if (c.closed) {
      appendDevLog({ ...base, field: 'closed', from: 'відкритий', to: 'закритий' });
    } else {
      if (String(c.nw) !== String(c.p.wagon))
        appendDevLog({ ...base, field: 'wagon', from: c.p.wagon, to: c.nw });
      if (String(c.nd) !== String(c.p.doors))
        appendDevLog({ ...base, field: 'doors', from: c.p.doors, to: c.nd });
    }
  });
...
invalidateLocalEditsCache();
applyLocalEdits(state.stationsData);
...
MetroApp.triggerFeedbackSubmit = triggerFeedbackSubmit;
...
editBtn.innerHTML = MetroApp.Icons?.pencil || PENCIL_SVG_LABEL;
...
if (state.stationsData) applyExitLabels(state.stationsData);
MetroApp.refreshCurrentStation?.();
markFeedbackDirty();
}
}
...
const changeStationBtn = document.getElementById('fbChangeStation');
if (changeStationBtn) changeStationBtn.hidden = true;
...
const allFilterBtn = document.querySelector('#fbLineFilter .search-line-btn[data-line=""]');
